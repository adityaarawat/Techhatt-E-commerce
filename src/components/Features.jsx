    ...
    <div className='py-8 px-4 sm:px-6 lg:px-8'>
        ...
        {features.map((feature,index)=>{
            return <div key={index} className='flex items-center justify-center text-center sm:text-left'>
                <feature.icon className='flex-shrink-0 h-10 w-10 text-red-500' aria-hidden="true"/>
                <div className='ml-4'>
                    <p className='text-base font-medium text-red-500'>{feature.text}</p>
                    <p className='mt-1 text-sm text-red-500'>{feature.subtext}</p>
                </div>
            </div>
        })
